...
namespace CraigsCreations.com.X10 {
	...
	/// </summary>
	public class X10CM11aController : IX10Controller {
		...
		public X10CM11aController(HouseCode houseCode,string comPort,int baudRate,Parity parity,int dataBits,StopBits stopBits) {
			try {
		...
		public X10CM11aController(HouseCode houseCode,string comPort) : this(houseCode,comPort,4800,Parity.None,8,StopBits.One) { }
		...
		private void Send(byte[] buffer,int count) {
			...
			do {
				...
				if(0x5a==retVal) { //If status messages are being transmitted cmd will not go through
					byte[] sendpollack = new byte[1]; 
					sendpollack[0] = 0xc3;
					comm.Write(sendpollack,0,1); //Send cmd to acknowledged status command.
				}
				x++;
			}while( (Checksum(buffer,count) != retVal) && TransmissionRetries > x); //Try to resend transmission of it did not go through

			//If too many retried send exception
			if(TransmissionRetries <= x) { throw(new System.ApplicationException("To Many Retries")); }
		...
		public int TransmissionRetries {
			get {
				return(transmissionRetries);
			}
			set {
				transmissionRetries = value;
			}
		}
